...
DFSで頂点に最初に出会った時点でスタックに積んでいき、再帰呼出しが帰りがけにスタックの一番上を出力すると一列化の逆順。
...
まず任意の点からDFSして訪問順に番号をふる。またその時にできるDFS TreeをTとする。この時、自分自身の番号と、Tに属さないエッジがある時のその先の番号、及び自分のすべての子供のlowestの仲の最小値をlowestと定義する。すると二つ以上の子供をもつルートと自分の親の番号とlowestの大小が反転している点が関節点になる。
